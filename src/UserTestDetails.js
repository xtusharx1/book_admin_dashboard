...
import { Link, useParams } from 'react-router-dom'; // Import Link and useParams for getting user ID
import './PracticeTest.css'; // Use similar CSS for consistency

function UserTestDetails() {
  const { u_id } = useParams(); // Get the user ID from the route parameters
  ...
  useEffect(() => {
    // Fetch test data for a specific user when component mounts
    const fetchTests = async () => {
      try {
        // Fetch tests for the specific user by u_id
        const response = await axios.get(`http://ec2-13-202-53-68.ap-south-1.compute.amazonaws.com:3000/api/tests/user/${u_id}`);
  ...
  return (
    <div className="test-list-container">
      ...
      {filteredTests.length === 0 ? (
        <p>No tests available for this user.</p>
      ) : (
          ...
          <tbody>
            {filteredTests.map((test, index) => (
              <tr key={test.test_id} style={{ color: 'grey' }}>
                ...
                <td>{test.userName}</td>
                <td>{test.bookName}</td>
                <td>{test.chapterNames}</td>
                <td>{test.chapterCount}</td>
                <td>{test.total_questions}</td>
                <td>{test.correct_answers}</td>
                <td>{test.wrong_answers}</td>
                <td>{formatTimeTaken(test.time_taken)}</td>
                <td>{test.result}%</td>
